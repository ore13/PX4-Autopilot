...
void FuseAirspeed(const matrix::Matrix<Scalar, 3, 1>& v_local,
                  ...
                  matrix::Matrix<Scalar, 3, 1>* const K = nullptr,
                  Scalar* const innov_var = nullptr, Scalar* const innov = nullptr) {
  // Total ops: 56
  ...
  const Scalar _tmp2 = sqrt(Scalar(pow(_tmp0, Scalar(2)) + pow(_tmp1, Scalar(2)) + epsilon +
                                   pow(v_local(2, 0), Scalar(2))));
